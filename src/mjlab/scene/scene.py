...
class SceneCfg:
  ...
  physics_factory: tyro.conf.Suppress[
    tyro.conf.Fixed[Callable[[int], tuple[PhysicsExtension, ...]] | None]
  ] = None
  """Factory for composable physics systems attached to this scene."""
...
class Scene:
  def __init__(self, scene_cfg: SceneCfg, device: str) -> None:
    ...
    self._physics = (
      scene_cfg.physics_factory(scene_cfg.num_envs) if scene_cfg.physics_factory else ()
    )
  ...
  @property
  def physics(self) -> tuple[PhysicsExtension, ...]:
